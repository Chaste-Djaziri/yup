  ...
  try {
    ...
    if (data.status === "published" && isResendEnabled()) {
      ...
      if (cfg.communitySegmentId) {
        const broadcast = await resend.broadcasts.create({ segmentId: cfg.communitySegmentId, from: cfg.from, subject: `Event Update: ${data.title}`, html: `<h2>${data.title}</h2><p>${data.summary ?? ""}</p><p>${data.description ?? ""}</p><p><a href="${data.registration_url ?? "https://yupinitiative.com/events"}">Register</a></p>` });
        if (broadcast.data?.id) {
          await resend.broadcasts.send(broadcast.data.id, {});
          await supabase.from("email_logs").insert({ event_type: "event_broadcast", subject: `Event Update: ${data.title}`, status: "sent", provider_message_id: broadcast.data.id, payload: { event_id: data.id } });
        }
      }
    }
