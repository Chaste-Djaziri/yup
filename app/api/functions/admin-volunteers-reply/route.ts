  ...
  try {
    ...
    if (isResendEnabled()) {
      ...
      const sent = await runResendSafe(() =>
        resend.emails.send({ from: senderFrom("volunteer"), to: data.email, subject, html: `<p>Hello ${data.first_name},</p><p>${body.message}</p>` }),
      );
      await supabase.from("email_logs").insert({
        event_type: "volunteer_reply_email",
        recipient_email: data.email,
        subject,
        provider_message_id: sent.ok ? (sent.data as any)?.data?.id ?? null : null,
        status: sent.ok ? "sent" : "failed",
        payload: { volunteer_id: data.id, error: sent.ok ? null : sent.error },
      });
    }
