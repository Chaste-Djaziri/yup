...
export async function POST(req: Request) {
  try {
    ...
    const { data, error } = await supabase.from("partner_submissions").select("*").eq("id", body.id).single();
    if (error) throw error;

    const subject = body.subject || `Reply: Partnership Inquiry from ${data.organization_name}`;

    await supabase
      .from("partner_submissions")
      .update({ admin_reply: body.message, replied_at: new Date().toISOString() })
      .eq("id", body.id);
