...
const eventImage = (event: DbEvent) => event.image_url || "/yup-assets/event.jpg";
...
export default async function HomePage() {
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="bg-background py-16"><div className="container mx-auto px-4 lg:px-8"><h2 className="text-center font-heading text-4xl text-primary">Upcoming Events</h2><div className="mt-8 grid gap-6 md:grid-cols-2">{upcomingEvents.length === 0 && (<div className="md:col-span-2"><EmptyStatePanel title="No Events Visible Yet" description="There are no upcoming published events at the moment. New events will appear here automatically." actionLabel="Visit Events" actionHref="/events" compact /></div>)}{upcomingEvents.map((event) => (<article key={event.slug} className="overflow-hidden bg-card"><img src={eventImage(event)} alt={event.title} className="h-44 w-full object-cover" /><div className="p-6"><p className="text-xs font-bold uppercase tracking-wider text-primary">{formatEventDate(event)}</p><h3 className="mt-2 font-heading text-2xl">{event.title}</h3><p className="mt-2 text-foreground/80">{event.summary || "No summary available."}</p><p className="mt-2 text-sm text-foreground/60">{event.location || "Location TBD"}</p><Link href={`/events/${event.slug}`} className="mt-4 inline-block text-sm font-semibold uppercase tracking-wider text-primary">Event Details</Link></div></article>))}</div><div className="mt-8 text-center"><Link href="/events" className="bg-primary px-7 py-3 text-xs font-bold uppercase tracking-wider text-primary-foreground">View All Events</Link></div></div></section>
