...
export default async function ProgramsPage() {
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="bg-background py-16"><div className="container mx-auto px-4 lg:px-8">{programs.length === 0 ? (<EmptyStatePanel title="No Programs Published Yet" description="No program is visible right now. Published programs will appear here automatically once added from the admin dashboard." actionLabel="Go To Volunteer" actionHref="/volunteer" />) : (<div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{programs.map((program) => (<article key={program.slug} className="overflow-hidden bg-card"><img src={program.cover_image_url || "/yup-assets/programs-header.jpg"} alt={program.title} className="h-52 w-full object-cover" /><div className="p-6"><p className="text-xs font-bold uppercase tracking-wider text-primary">{program.category}</p><h2 className="mt-2 font-heading text-2xl">{program.title}</h2><p className="mt-3 text-sm text-foreground/80">{program.summary || "No summary available."}</p><Link href={`/programs/${program.slug}`} className="mt-4 inline-block text-sm font-semibold uppercase tracking-wider text-primary">Learn More</Link></div></article>))}</div>)}</div></section>
